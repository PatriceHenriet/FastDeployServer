...
def get_memory_information():
    f = open("hw_inventory.txt", "a")
    response = requests.get('https://%s/redfish/v1/Systems/System.Embedded.1/Memory' % idrac_ip, verify=False,
                            auth=(idrac_username, idrac_password))
    data = response.json()
    if response.status_code != 200:
        print("\n- FAIL, get command failed, error is: %s" % data)
    ...
    for i in data['Members']:
        ...
        try:
            ...
            sys.exit()
        response = requests.get('https://%s%s' % (idrac_ip, i['@odata.id']), verify=False,
                                auth=(idrac_username, idrac_password))
...
def get_cpu_information():
    f = open("hw_inventory.txt", "a")
    response = requests.get('https://%s/redfish/v1/Systems/System.Embedded.1/Processors' % idrac_ip, verify=False,
                            auth=(idrac_username, idrac_password))
    data = response.json()
    if response.status_code != 200:
        print("\n- FAIL, get command failed, error is: %s" % data)
        ...
        if response.status_code != 200:
            ...
            for ii in sub_data.items():
                if ii[0] == '@odata.id' or ii[0] == '@odata.context' or ii[0] == 'Metrics' or ii[0] == 'Links' or ii[
                    0] == 'Description' or ii[0] == '@odata.type':
                    pass
                elif ii[0] == 'Oem':
...
def get_storage_controller_information():
    ...
    controller_list = []
    response = requests.get('https://%s/redfish/v1/Systems/System.Embedded.1/Storage' % idrac_ip, verify=False,
                            auth=(idrac_username, idrac_password))
    data = response.json()
    for i in data["Members"]:
        for ii in i.items():
...
def get_storage_disks_information():
    ...
    for i in controller_list:
        response = requests.get(
            'https://%s/redfish/v1/Systems/System.Embedded.1/Storage/%s' % (idrac_ip, i.split("/")[-1]), verify=False,
            auth=(idrac_username, idrac_password))
        data = response.json()
...
def get_backplane_information():
    f = open("hw_inventory.txt", "a")
    response = requests.get('https://%s/redfish/v1/Chassis' % (idrac_ip), verify=False,
                            auth=(idrac_username, idrac_password))
    data = response.json()
    if response.status_code != 200:
        print("\n- FAIL, get command failed, error is: %s" % data)
    ...
    for i in backplane_URI_list:
        ...
        for iii in data.items():
            if iii[0] == '@odata.id' or iii[0] == '@odata.context' or iii[0] == 'Metrics' or iii[0] == 'Links' or iii[
                0] == '@Redfish.Settings' or iii[0] == '@odata.type' or iii[0] == 'RelatedItem' or iii[
                0] == 'Actions' or iii[0] == 'PCIeDevices':
                pass
            else:
                message = "%s: %s" % (iii[0], iii[1])
                f.writelines(message)
    ...
    for i in network_URI_list:
        ...
        for z in port_uri_list:
            ...
            if response.status_code != 200:
                ...
                print(message)
                for ii in data.items():
                    if ii[0] == '@odata.id' or ii[0] == '@odata.context' or ii[0] == 'Metrics' or ii[0] == 'Links' or \
                            ii[0] == '@odata.type':
                        pass
                    elif ii[0] == 'Oem':
                        try:
